    ...
    def detect_tasks(self) -> list[tuple[TrailblazePowerFunc, dict[str, int]]] | None:
        ...
        try:
            ...
            if not res:
                ...
                self.operator.press_key("esc")
                return None
            # 合并所有 OCR 文本，用正则提取数字（兼容分开识别和合并识别两种情况）
            import re
            full_text = "".join(r[1] for r in res)
            logger.debug(f"体力 OCR 原始文本: {full_text}")

            # 提取所有 "数字/数字" 片段（当前体力和沉浸器都是这种格式）
            slash_numbers = re.findall(r'(\d+)/(\d+)', full_text)
            if len(slash_numbers) < 2:
                raise ValueError(f"未找到足够的 数字/数字 片段: {full_text}")

            current_tbp = int(slash_numbers[0][0])   # 第一个 x/y 的 x 是当前体力
            immersion_dev = int(slash_numbers[1][0])  # 第二个 x/y 的 x 是沉浸器

            # 后备体力：在第一个 "/" 之前、最后一段纯数字
            before_first_slash = full_text[:full_text.index('/')]
            reserve_candidates = re.findall(r'\d+', before_first_slash)
            if not reserve_candidates:
                raise ValueError(f"未找到后备体力数字: {before_first_slash}")
            # 当前体力的数字也在这段里（粘连情况），取最后一个纯数字前缀
            # 例如 "+79312" -> 当前体力是312，后备是79，但我们已经从slash_numbers拿到了312
            # 所以这里把 current_tbp 的字符串从末尾去掉，剩下的就是后备体力
            combined = reserve_candidates[-1]  # 例如 "79312"
            current_str = str(current_tbp)     # "312"
            if combined.endswith(current_str) and len(combined) > len(current_str):
                reserve_tbp = int(combined[:-len(current_str)])
            else:
                reserve_tbp = int(combined)
