...
class IOperator(ABC):
    ...
    def ocr_in_region(
        ...
        try:
            ...
            # RapidOCR 不接受 PIL Image，转为 numpy array
            import numpy as np
            screenshot_np = np.array(screenshot)
            result, _ = self.ocr_engine(screenshot_np, use_det=True, use_cls=False, use_rec=True)  # NOQA # type: ignore
    ...
    def ocr_match(self,
        ...
        if results is None:
            return None
        ocr_confidence = confidence if confidence is not None else load_settings().get('ConfidenceThreshold', 0.9)
        for result in results:
            if float(result[2]) >= ocr_confidence and text in result[1]:
                left, top = result[0][0]
